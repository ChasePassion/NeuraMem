"""LoCoMo benchmark prompt templates and helpers.

Judge prompts replicate the exact structure used by OpenViking's LoCoMo
benchmark; the answer prompt delegates to the library canonical builder
(neuramem.prompts.build_answer_prompt) — verified byte-identical against
a pre-delegation snapshot (reference_date="2023" reproduces the legacy
template exactly).
"""

import json

from neuramem.prompts import build_answer_prompt  # canonical single source
...
def get_answer_generation_prompt(
    ...
    """Delegate to the library canonical builder (single source, ch. 9).

    Keeps the benchmark semantics: reference_date defaults to "2023"
    (the library default is the current year -- product behavior).
    """
    if reference_date is None:
        reference_date = "2023"
    return build_answer_prompt(
        question=question, memories=search_results, reference_date=reference_date
    )


JUDGE_SYSTEM_PROMPT = """You are an accurate and strict evaluator for long-term conversational memory QA. Output JSON with fields 'reasoning' (string) and 'label' ('CORRECT' or 'WRONG')."""
...
def get_judge_prompt(
    ...
    del category
    return _JUDGE_TEMPLATE.format(question=question, answer=answer, response=response)
